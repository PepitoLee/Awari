...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onClick }) => {
  return (
    ...
    >
      <div className="aspect-square rounded-2xl overflow-hidden bg-wool-50 relative mb-4">
        <img
          src={product.image}
          alt={product.name}
        ...
        >
          <Plus className="w-5 h-5" style={{ color: '#5d4037' }} />
        </button>
      </div>
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-2">
          <div>
            <p className="text-xs font-bold text-blush-400 uppercase tracking-wider mb-1">{product.category}</p>
            <h3 className="font-display font-bold text-2xl text-wool-900 leading-tight group-hover:text-blush-400 transition-colors">{product.name}</h3>
          </div>
          <span className="font-display font-black text-xl text-wool-700 bg-wool-100 px-3 py-1 rounded-full">
            S/. {product.price.toFixed(2)}
          </span>
        </div>
        <p className="text-wool-500 text-sm line-clamp-2">{product.description}</p>
      </div>
